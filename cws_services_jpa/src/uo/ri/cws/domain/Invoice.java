...
public class Invoice extends BaseEntity {
    ...
    public Invoice(Long number, LocalDate date, List<WorkOrder> workOrders) {
        ...
        ArgumentChecks.isTrue(number >= 0, "Invalid number below 0");
    ...
    private void computeAmount() {
        ...
        for (WorkOrder workOrder : workOrders) {
            total += workOrder.getAmount();
        }

        double vatPercentage = -1;

        if (date.isAfter(LocalDate.of(2012, 7, 1))) {
            vatPercentage = 0.21;
        } else {
            vatPercentage = 0.18;
        }

        this.amount = Round.twoCents(total * (1 + vatPercentage));
        this.vat = Round.twoCents(total * vatPercentage);
    }
    ...
    public void settle() {
        StateChecks
            .isTrue(isSettled() || getTotalChargesAmount() >= this.amount);

        this.state = InvoiceState.PAID;
    }

    private double getTotalChargesAmount() {
        double total = 0.0;
